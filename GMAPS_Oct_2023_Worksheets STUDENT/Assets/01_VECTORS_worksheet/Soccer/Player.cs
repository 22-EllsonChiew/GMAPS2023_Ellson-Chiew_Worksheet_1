...
public class Player : MonoBehaviour
{
    ...
    float AngleToPlayer()
    {
        ...
        //get direction of the otherplayer from captain
        Vector3 directionPlayer = (OtherPlayer.transform.position - transform.position);

        //float dotProduct = Dot(transform.forward, directionPlayer);

           
        DebugExtension.DebugArrow(transform.position, directionPlayer / 1f, Color.black);
        DebugExtension.DebugArrow(transform.position, transform.forward * 3f, Color.blue);
    ...
    void Update()
    {
        if (IsCaptain)
        {
            Vector3 directionPlayer = OtherPlayer.transform.position;
            float angle = Mathf.Atan2(directionPlayer.z, directionPlayer.x) * Mathf.Rad2Deg;
            Debug.Log(angle);
